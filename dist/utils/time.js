...
exports.parseIsoWithOffsetOrThrow = parseIsoWithOffsetOrThrow;
exports.parseAndValidateBookingRange = parseAndValidateBookingRange;
exports.halfOpenOverlaps = halfOpenOverlaps;
exports.businessHoursBetweenHalfOpen = businessHoursBetweenHalfOpen;
exports.overlapHoursHalfOpen = overlapHoursHalfOpen;
...
function parseAndValidateBookingRange(params) {
    ...
    const endLocal = end;
    const isWeekday = (d) => d.weekday >= 1 && d.weekday <= 5; // Mon=1 ... Sun=7
...
function halfOpenOverlaps(aStart, aEnd, bStart, bEnd) {
    const aS = luxon_1.DateTime.fromISO(aStart, { setZone: true }).toMillis();
    const aE = luxon_1.DateTime.fromISO(aEnd, { setZone: true }).toMillis();
    const bS = luxon_1.DateTime.fromISO(bStart, { setZone: true }).toMillis();
    const bE = luxon_1.DateTime.fromISO(bEnd, { setZone: true }).toMillis();
    return aS < bE && aE > bS;
}
function businessHoursBetweenHalfOpen(fromIso, toIso) {
    ...
    while (cursor.toMillis() < endMs) {
        ...
        if (weekday >= 1 && weekday <= 5) {
            ...
            const intervalStart = luxon_1.DateTime.max(windowStart, from);
            const intervalEnd = luxon_1.DateTime.min(windowEnd, to);
...
function overlapHoursHalfOpen(aStartUtcIso, aEndUtcIso, fromUtcIso, toUtcIso) {
    const aS = luxon_1.DateTime.fromISO(aStartUtcIso, { setZone: true }).toMillis();
    const aE = luxon_1.DateTime.fromISO(aEndUtcIso, { setZone: true }).toMillis();
    const bS = luxon_1.DateTime.fromISO(fromUtcIso, { setZone: true }).toMillis();
    const bE = luxon_1.DateTime.fromISO(toUtcIso, { setZone: true }).toMillis();
