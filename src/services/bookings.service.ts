...
import { createIdempotencyRepo } from '../persistence/idempotency.repo';
import { createRoomRepo } from '../persistence/room.repo';
import type { Booking } from '../models/booking';
import { parseAndValidateBookingRange } from '../utils/time';

export function createBookingsService(db: FileDb) {
  ...
  return {
    ...
    listBookings: (params: {
      ...
      let fromUtcIso: string | undefined;
      let toUtcIso: string | undefined;

      const parseIsoToUtc = (iso: string, field: string) => {
    ...
    cancelBooking: (params: { bookingId: string }): Booking => {
      return db.withWriteLock(() => {
